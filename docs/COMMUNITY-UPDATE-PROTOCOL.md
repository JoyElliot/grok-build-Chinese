...
- 每个平台一份完整归档；历史三平台 Release 有三份归档和三个独立 `.sha256`，六平台正式版有六份归档和六个独立 `.sha256`。
...
旧现代客户端遇到六平台发布版时，可能因附件总集合从六个变为十二个而跳过，继续选择 `B`，下一次由 `B` 中的新引擎更新到 `N`。若以后停发 sidecar，旧客户端也会跳过缺 sidecar 的新 Release。这里的“过渡结束”表示停止生成新的旧式发布附件，不是删除桥接发布。更早只认识裸 EXE 的预览版仍需按原说明手动迁移。
