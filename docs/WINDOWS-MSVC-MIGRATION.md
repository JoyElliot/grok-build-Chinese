...
两次并发首次启动使用入口旁的 `grok-zh.exe.update.lock` 独占锁；第二个进程有界等待，迁移成功后复用已安装程序。运行目录的版本检查与恢复升级另取 `.grok-zh-msvc/grok-zh.exe.update.lock`，与当前 MSVC Rust 更新器协调。已发布的旧 GNU 更新器并非全部实现这个锁协议，不能声称新锁能约束旧代码。不会尝试重命名包含正在运行启动器的目录。

若旧更新器后来只将入口换成更高版本的 GNU 启动器，已有的受管 MSVC 目录会进入恢复升级：重新核验归属与目标架构，下载并完整验证新入口对应的同版本 MSVC 包，再按其 `executable-only` 协议仅替换运行 EXE。锁内重新检查实际版本，保留已经更新到更高版本的程序；安装元数据、支持文件和个人文件保持原位，与正常 MSVC 自更新一致。完成后才写新 ready 标记，此后恢复无下载启动。

## 失败与兼容边界

- 第一次迁移需要网络、Windows PowerShell 5.1 和当前安装目录的写权限。下载、摘要或安装失败时，保留 GNU 入口以便重试；旧完整应用此前已被旧更新器替换，不能保证迁移失败时旧应用仍可离线使用。
- 现有无效、未知或非 x64 MSVC 的 `.grok-zh-msvc` 目录仍拒绝覆盖。有效旧版本可自动恢复升级；下载失败或 EXE 无法替换时保留原程序以供重试，不截断运行中的 EXE。
- 使用同卷文件替换及独立随机备份；Windows 1177 部分失败时校验旧备份摘要再恢复原 EXE。若连恢复也受权限/占用阻挡，保留候选与备份并报告路径。被旧会话占用的备份不强删、不终止会话；备份名避开 Rust 普通 `.old` 清理规则。
...
- `Test-WindowsMigration.ps1`：真实 GNU C 入口配测试运行时，覆盖纯版本查询、首启下载、后续离线启动、并发、错误摘要/重试、参数/标准流/退出码、保留用户数据和 PATH；另覆盖 1.0.98→1.0.99 只换入口的恢复升级、运行目录锁等待、运行中旧会话、错误摘要、拒绝降级/未知目录及模拟 1177/恢复失败。测试网络传输只在测试启动器编译时注入，生产程序没有环境变量 URL 或信任绕过开关。
- `Test-MigrationProduct.ps1`：使用本轮真正编译的 MSVC 产品包和 helper，通过离线 HTTP fixture 验证旧 GNU ZIP 的 EXE-only 边界到完整安装，再执行四类 PATH 隔离的 CLI 检查。可用 `-PreviousPackageDirectory` 加载另一个旧 CI 真实 MSVC 包，先完成旧版迁移，再仅换新 GNU EXE 并验证实际产品恢复升级。它不证明 GitHub 上尚未发布的 helper 可下载，也不直接执行历史 Rust 客户端的更新流程。
...
## 2026-09-27：入口升级后恢复已迁移的旧运行目录

本机先用真实 GNU 启动器迁移到测试运行时 1.0.98，再只复制 1.0.99 的 GNU 入口，原实现稳定报“已有未完成或较旧的迁移运行目录”。修复后自动完成恢复升级，两个版本的候选 `--version` 仍无下载副作用。PowerShell 7/5.1 各 **57 项**回归通过，涵盖更新锁、下载失败后重试、摘要绑定、拒绝降级/未知目录/错误平台、运行中旧会话以及模拟 Windows 1177 回滚与回滚受阻保留文件。

另下载历史 CI 36236500352 的真实 MSVC **1.0.100** 与 CI 36242349101 的真实 **1.0.102** 包，在本机 PowerShell 7/5.1 分别完成初次旧版迁移→只换新 GNU 入口→实际 MSVC 1.0.102 恢复升级。最终 EXE 摘要与新 CI 产品相同，四类系统 PATH 隔离 CLI 通过，runtime 内个人文件和共享数据保留，后续不重复下载。HTTP 仍为测试编译时注入，旧 Rust 的只替换 EXE 步骤仍模拟，没有发布 helper 或隔离版本。

14 项迁移打包/工作流/输入绑定 Python 测试、actionlint 与 diff 检查通过；独立增量审查提出的锁检查、1177 部分失败恢复及候选摘要绑定均已处理并回归。新的整轮 CI 使用隔离产品 **1.0.103**，当前修复提交仍需完整 CI 验收，不能沿用旧 head 5fd05117 的 **38 分 45 秒**作为通过证明。
...
### 1.0.101 复验：迁移成功，整轮因 Intel 打包失败

提交 `b390af67` 的 [CI 36239557884](https://github.com/JoyElliot/grok-build-Chinese/actions/runs/36239557884) 从 `11:40:28Z` 至 `12:27:03Z`，共 **46 分 35 秒**，结论为失败，不能计作第二次完整达标。MSVC 产品 `1.0.101` 的交叉构建（33 分 32 秒）及独立 x64 实际迁移（1 分 27 秒）通过，原生 updater/fixture 也通过；失败来自 macOS Intel 裁剪后程序首次 `--version` 的 30 秒超时，其原生产物验收因此未执行，汇总正确阻断。

修复范围与诊断边界见 [macOS 启动超时记录](MACOS-CI-PERFORMANCE.md#新版本复验中的-rosetta-启动超时)。下一轮仍执行全部测试和产品验证，将 MSVC 产品改成新的隔离版本 `1.0.102`，避免同版本成品复用掩盖重编译；纯 fixture 继续固定 `1.0.99`。缓存布局、编译器与生产优化参数保持。仍需修复后的当前 head 全部成功且满足总时限。
