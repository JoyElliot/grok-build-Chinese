...
'@
    ...
    if (!$versionOutput.Contains("grok-zh $testVersion (GNU migration launcher)") -or (Test-Path -LiteralPath $log)) { throw '旧更新器版本检查触发了迁移或版本不匹配。' }
    # All toolchain directories are absent from PATH for native CLI acceptance.
    $env:Path="$env:SystemRoot\System32;$env:SystemRoot"
    $help=Run-Product $launcher '--help'
    if (!$help -or @(Get-Content -LiteralPath $log).Count -ne 4) { throw '首次迁移未完整执行，或发生重复下载。' }
    ...
    if (@(Get-Content -LiteralPath $log).Count -ne 4) { throw '后续CLI启动又下载了迁移包。' }
