...
impl SentMessageTarget {
    ...
    fn noun_with_locale(&self, locale: &crate::locale::LocaleContext) -> String {
        match self {
            Self::Named { label, .. } => scrub_display(label).into_owned(),
            Self::Parent => locale
                .named_text("scrollback.sent_message.parent", PARENT_NOUN)
                .into_owned(),
            Self::Unresolved { .. } => {
                let english = self.noun();
                let prefix = locale.named_text("scrollback.sent_message.subagent", FALLBACK_NOUN);
                format!(
                    "{prefix}{}",
                    english.strip_prefix(FALLBACK_NOUN).unwrap_or(&english)
                )
            }
        }
    }
...
impl SentMessageToolCallBlock {
    ...
    fn verb_and_target(&self) -> String {
        ...
        match &self.presentation {
            ...
            | SentMessagePresentation::RejectedUnavailable => format!("rejected \u{00b7} {noun}"),
    ...
    fn verb_and_target_with_locale(&self, locale: &crate::locale::LocaleContext) -> String {
        let noun = self.input.as_ref().map_or_else(
            || {
                locale
                    .named_text("scrollback.sent_message.subagent", FALLBACK_NOUN)
                    .into_owned()
            },
            |input| input.target.noun_with_locale(locale),
        );
        let (id, english) = match &self.presentation {
            SentMessagePresentation::Sending => {
                ("scrollback.sent_message.sending_to", "sending to {target}")
            }
            SentMessagePresentation::Sent => {
                match self.input.as_ref().and_then(|input| input.delivery) {
                    None | Some(SentMessageDelivery::Steer) => {
                        ("scrollback.sent_message.sent_to", "sent to {target}")
                    }
                    Some(SentMessageDelivery::Queue) => {
                        ("scrollback.sent_message.queued_for", "queued for {target}")
                    }
                    Some(SentMessageDelivery::Interject) => (
                        "scrollback.sent_message.interjected_to",
                        "interjected to {target}",
                    ),
                }
            }
            SentMessagePresentation::Rejected { .. }
            | SentMessagePresentation::RejectedUnavailable => (
                "scrollback.sent_message.rejected_target",
                "rejected · {target}",
            ),
            SentMessagePresentation::Unconfirmed { .. } => (
                "scrollback.sent_message.unconfirmed_target",
                "unconfirmed · {target}",
            ),
        };
        locale.named_text(id, english).replace("{target}", &noun)
    }
    ...
    fn collapsed_line(&self, ctx: &BlockContext, theme: &Theme) -> Line<'static> {
        ...
        Line::from(vec![
            Span::styled(
                ctx.locale
                    .named_static_text("scrollback.sent_message.label", HEADER_LABEL),
                label_style,
            ),
    ...
    fn expanded_header(&self, theme: &Theme) -> Line<'static> {
        self.expanded_header_with_locale(theme, &crate::locale::LocaleContext::default())
    }

    fn expanded_header_with_locale(
        &self,
        theme: &Theme,
        locale: &crate::locale::LocaleContext,
    ) -> Line<'static> {
        let mut spans = vec![
            Span::styled(
                locale.named_static_text("scrollback.sent_message.label", HEADER_LABEL),
                theme.primary().add_modifier(Modifier::BOLD),
            ),
            Span::styled(self.verb_and_target_with_locale(locale), theme.primary()),
        ];
        if let Some(delivery) = self.input.as_ref().and_then(|input| input.delivery) {
            let delivery = match delivery {
                SentMessageDelivery::Steer => {
                    locale.named_static_text("scrollback.sent_message.delivery.steer", "steer")
                }
                SentMessageDelivery::Queue => {
                    locale.named_static_text("scrollback.sent_message.delivery.queue", "queue")
                }
                SentMessageDelivery::Interject => locale
                    .named_static_text("scrollback.sent_message.delivery.interject", "interject"),
            };
...
impl BlockContent for SentMessageToolCallBlock {
    fn output(&self, ctx: &BlockContext) -> BlockOutput {
        ...
        let mut lines: Vec<BlockLine> =
            vec![self.expanded_header_with_locale(&theme, &ctx.locale).into()];
