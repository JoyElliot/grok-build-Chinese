...
fn empty_state_markdown(capture_enabled: bool, dream_enabled: bool) -> String {
    empty_state_markdown_with_locale(capture_enabled, dream_enabled, None)
}

fn empty_state_markdown_with_locale(
    ...
    if capture_enabled {
        text.push_str(memory_static(
            locale,
            "memory.empty.capture",
...
fn disabled_state_markdown_with_locale(
    reason: Option<MemoryDisabledReason>,
    locale: Option<&LocaleContext>,
) -> String {
    let key = match reason {
        None | Some(MemoryDisabledReason::SessionToggle) => "memory.disabled.session",
        Some(MemoryDisabledReason::ConfigOptOut) => "memory.disabled.config",
        Some(MemoryDisabledReason::ProcessDisabled) => "memory.disabled.process",
        Some(MemoryDisabledReason::RolloutRestricted) => "memory.disabled.rollout",
        Some(MemoryDisabledReason::NotConfigured) => "memory.disabled.not_configured",
        Some(MemoryDisabledReason::Unknown) => "memory.disabled.unknown",
    };
    memory_text(locale, key, disabled_state_markdown(reason))
}
...
pub struct MemoryModalState {
    ui_locale: Option<LocaleContext>,
...
impl MemoryModalState {
    ...
    pub fn from_listing(listing: MemoryListing) -> Self {
        Self::from_listing_with_locale(listing, None)
    }

    pub fn from_listing_with_locale(
        listing: MemoryListing,
        locale: Option<&LocaleContext>,
    ) -> Self {
        let mut state = Self::new(Vec::new());
        state.ui_locale = locale.cloned();
    ...
    pub fn apply_listing(&mut self, listing: MemoryListing) {
        ...
        self.entries = build_entries_with_locale(listing.files, self.ui_locale.as_ref());
...
pub fn build_entries_with_locale(
    ...
    for f in files {
        let label = entry_label_with_locale(&f, locale);
...
pub fn render_memory_modal_with_locale(
    ...
    if state.shows_notice() {
        ...
        let markdown = if state.memory_enabled {
            empty_state_markdown_with_locale(state.capture_enabled, state.dream_enabled, locale)
        } else {
            disabled_state_markdown_with_locale(state.disabled_reason, locale)
        };
...
fn render_file_list_with_locale(
    ...
    if state.filter_has_no_matches() {
        render_no_matches(buf, area, entries_start_y, state.query(), theme, locale);
        return;
    }
...
fn render_no_matches(
    ...
    let lines = [
        memory_text(locale, "memory.filter.no_match", "No notes match “{query}”")
            .replace("{query}", query),
        memory_text(locale, "memory.filter.clear", "Backspace clears the filter"),
    ];
    ...
    match key.code {
        ...
        KeyCode::Char('x') if key.modifiers.is_empty() => {
            ...
            let scope = match entry.source.as_str() {
                "session" => memory_text(
                    state.ui_locale.as_ref(),
                    "memory.scope.session_logs",
                    "session logs",
                ),
                "workspace" => memory_text(
                    state.ui_locale.as_ref(),
            ...
            state.status = Some(MemoryStatusLine {
                text: memory_text(state.ui_locale.as_ref(), "memory.delete.confirm", "Delete {label} from {scope}? Dream may re-derive it from future sessions. x confirm · any other key cancels")
                    .replace("{label}", &entry.label).replace("{scope}", &scope),
                is_error: false,
                ticks_remaining: None,
            });
...
fn build_shortcuts_with_locale(
    ...
    let plain = |label: &'static str| Shortcut {
        label: localized_memory_shortcut(label, locale),
        clickable: false,
        id: 0,
    };
    ...
    match &state.mode {
        ...
        MemoryModalMode::Browse => {
            ...
            modal_window::push_vim_nav_search_hint_with_locale(&mut shortcuts, false, locale);
            shortcuts
        }
...
fn localized_memory_shortcut(label: &'static str, locale: Option<&LocaleContext>) -> &'static str {
    let key = match label {
        "t turn on" => "memory.shortcut.turn_on",
        "t turn off" => "memory.shortcut.turn_off",
        "^F normal" => "memory.shortcut.normal",
        "^F fullscreen" => "memory.shortcut.fullscreen",
        "Esc close" => "memory.shortcut.close",
        "\u{2191}/\u{2193} scroll" => "memory.shortcut.scroll",
        "drag to copy" => "memory.shortcut.drag_copy",
        "Esc list" => "memory.shortcut.list",
        "Esc back" => "memory.shortcut.back",
        "\u{2191}/\u{2193} nav" => "memory.shortcut.navigate",
        "/ search" => "memory.shortcut.search",
        "Enter read" => "memory.shortcut.read",
        "Enter open" => "memory.shortcut.open",
        "y copy path" => "memory.shortcut.copy_path",
        "x delete" => "memory.shortcut.delete",
        "s settings" => "memory.shortcut.settings",
        "type to filter" => "memory.shortcut.type_filter",
        "Esc exit filter" => "memory.shortcut.exit_filter",
        "x confirm delete" => "memory.shortcut.confirm_delete",
        "any key cancel" => "memory.shortcut.any_cancel",
        "i search" => "memory.shortcut.search_vim",
        _ => return label,
    };
    memory_static(locale, key, label)
}
...
fn entry_label(f: &xai_grok_shell::extensions::notification::MemoryFileInfo) -> String {
    entry_label_with_locale(f, None)
}

fn entry_label_with_locale(
    f: &xai_grok_shell::extensions::notification::MemoryFileInfo,
    locale: Option<&LocaleContext>,
) -> String {
    ...
    observation_key_label_with_locale(&name, locale).unwrap_or(name)
}

fn observation_key_label(file_name: &str) -> Option<String> {
    observation_key_label_with_locale(file_name, None)
}

fn observation_key_label_with_locale(
    file_name: &str,
    locale: Option<&LocaleContext>,
) -> Option<String> {
    ...
    let turns = if from == through {
        memory_text(locale, "memory.observation.turn", "turn {from}")
            .replace("{from}", &from.to_string())
    } else {
        memory_text(locale, "memory.observation.turns", "turns {from}–{through}")
            .replace("{from}", &from.to_string())
            .replace("{through}", &through.to_string())
    };
    Some(
        memory_text(
            locale,
            "memory.observation.label",
            "observation, {turns} (#{ordinal})",
        )
        .replace("{turns}", &turns)
        .replace("{ordinal}", &ordinal.saturating_add(1).to_string()),
    )
}
