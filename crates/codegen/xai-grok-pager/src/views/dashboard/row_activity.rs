        ...
        RowState::NeedsInput => {
            if let Some(perm) = agent.permission_queue.front() {
                ...
                if !title.is_empty() {
                    ...
                    return Some(
                        text(locale, "dashboard.row.pending", "Pending: {detail}")
    ...
    match state {
        ...
        RowState::Working if has_live_parent_activity(agent) => {
            if let Some(cmd) = agent.session.state.command_in_flight() {
                ...
                Some(text(locale, "dashboard.row.loading", "Loading…").to_owned())
            } else {
                Some(text(locale, "dashboard.row.working", "Working").to_owned())
            }
        }
        RowState::Working
        | RowState::Idle
...
fn text(locale: Option<&LocaleContext>, id: &str, english: &'static str) -> &'static str {
    locale
