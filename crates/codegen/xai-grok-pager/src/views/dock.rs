...
pub fn render_with_locale(
    ...
    for visual in visual_rows(&counts) {
        ...
        match visual {
            ...
            Visual::More(n) => {
                let more = locale
                    .map(|locale| locale.named_text("dock.more", "{count} more"))
                    .unwrap_or_else(|| std::borrow::Cow::Borrowed("{count} more"))
                    .replace("{count}", &n.to_string());
                let line = Line::from(Span::styled(
                    format!("{MORE_INDENT}▾ {more}"),
...
fn paint_row(
    ...
    if selected {
        ...
        if row.killable {
            let stop = locale
                .map(|locale| locale.named_static_text("turn.button.stop_spaced", " [stop]"))
                .unwrap_or(" [stop]");
            meta_spans.push(Span::styled(stop, Style::default().fg(theme.accent_error)));
        }
    }
...
mod tests {
    ...
    #[test]
    fn zh_localization_dock_chrome_preserves_dynamic_row_text() {
        let theme = Theme::tokyonight();
        let mut data = sample();
        data.focused = true;
        data.cursor = 1;
        ...
        render_with_locale(&mut buf, area, &theme, &data, Some(&locale));

        let header = row_text(&buf, 0);
        assert!(header.starts_with("▾ 子智能体 3 ─"), "{header:?}");
        let first = row_text(&buf, 1);
        assert!(
            first.contains("Explore find dashboard render path — reading render.rs"),
            "{first}"
        );
        assert!(first.trim_end().ends_with("grok-4.5 2m14s [↗] [停止]"));
        assert!(row_text(&buf, 3).contains("▾ 另有 1 项"));
        assert!(row_text(&buf, 4).starts_with("▸ 任务 1 ─"));
        assert!(row_text(&buf, 5).starts_with("▸ 监视器 2 ─"));
        assert!(row_text(&buf, 6).starts_with("▸ 队列 2 ─"));
    }
