...
impl SessionRpcError {
    pub(crate) fn localized_message(&self, locale: &crate::locale::LocaleContext) -> String {
        match self {
            Self::Rpc(error) => error.to_string(),
            Self::TimedOut { action, timeout } => {
                let action = match action.as_str() {
                    "Session creation" => locale.named_text("session.rpc.action.create", "Session creation"),
                    "Session loading" => locale.named_text("session.rpc.action.load", "Session loading"),
                    "Worktree session resume" => locale.named_text("session.rpc.action.resume", "Worktree session resume"),
                    other => std::borrow::Cow::Borrowed(other),
                };
                localized_named(locale, "session.rpc.timed_out",
                    "{action} timed out after {seconds}s. It may still finish in the background; retrying right away can run into the same delay.",
                    &[("action", action.as_ref()), ("seconds", &timeout.as_secs().to_string())])
            }
        }
    }
