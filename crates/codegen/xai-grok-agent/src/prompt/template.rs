...
mod tests {
    ...
    #[test]
    fn compact_prompt_preserves_plan_language_and_canonical_statuses() {
        assert!(
            COMPACT_SYSTEM_PROMPT.contains("For Chinese requests, use concise Simplified Chinese")
        );
        for status in ["pending", "in_progress", "completed", "cancelled"] {
            assert!(COMPACT_SYSTEM_PROMPT.contains(status));
        }
    }
