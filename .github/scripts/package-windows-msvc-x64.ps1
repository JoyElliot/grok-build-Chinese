# Package the native x64 MSVC executable with the same Windows layout and
...
function Assert-X64PE([string]$Path) {
    ...
    if ($machine -ne 0x8664 -or $optional -ne 0x20b) {
        throw "不是 x64 PE32+：$Path (machine=$machine)"
    }
}

if ($env:TARGET -cne 'x86_64-pc-windows-msvc' -or
    $env:PACKAGE_NAME -cne "grok-zh-$env:GROK_VERSION-windows-x86_64-msvc" -or
...
if (!(Test-Path -LiteralPath $built -PathType Leaf)) { throw "缺少 x64 构建结果：$built" }
Assert-X64PE $built
...
foreach ($arguments in @(
    ...
    if ($LASTEXITCODE -ne 0 -or !$output) { throw "x64 程序冒烟失败：$($arguments -join ' ')" }
    if ($arguments.Count -eq 1 -and $arguments[0] -eq '--version' -and
        $output -notmatch "^grok-zh $([regex]::Escape($env:GROK_VERSION)) \(") {
        throw "x64 程序版本不一致：$output"
    }
}
} finally { $env:Path = $toolPath }
...
$rgVersion = '15.1.0'
$rgArchive = Join-Path $env:RUNNER_TEMP 'ripgrep-x64-msvc.zip'
$rgRoot = Join-Path $env:RUNNER_TEMP 'ripgrep-x64-msvc'
Invoke-WebRequest -Uri "https://github.com/BurntSushi/ripgrep/releases/download/$rgVersion/ripgrep-$rgVersion-x86_64-pc-windows-msvc.zip" -OutFile $rgArchive -MaximumRetryCount 3 -RetryIntervalSec 5 -TimeoutSec 120
$rgHash = (Get-FileHash -LiteralPath $rgArchive -Algorithm SHA256).Hash
if ($rgHash -cne '124510B94B6BAA3380D051FDF4650EAA80A302C876D611E9DBA0B2E18D87493A') {
    throw "x64 ripgrep ZIP SHA-256 不匹配：$rgHash"
}
...
if (!$rgExe) { throw 'x64 ripgrep ZIP 缺少 rg.exe。' }
Assert-X64PE $rgExe.FullName
...
foreach ($name in @('COPYING', 'LICENSE-MIT', 'UNLICENSE')) {
    ...
    if (!$source) { throw "x64 ripgrep ZIP 缺少许可证：$name" }
...
$buildInfo = @"
Grok Build 简体中文 Windows x64 MSVC
Distribution: migration CI experiment only; not a published Release
...
if ($LASTEXITCODE -ne 0) { throw '生成 x64 包内更新协议失败。' }
...
if (($actualNames -join "`n") -cne ($expectedNames -join "`n")) {
    throw "x64 包文件集合不精确：$($actualNames -join ', ')"
}
...
if ((Get-Item -LiteralPath $zip).Length -gt 536870912L) {
    throw 'x64 ZIP 超过更新器 512 MiB 上限。'
}
$verifyRoot = Join-Path $env:RUNNER_TEMP "grok-zh-x64-msvc-verify-$env:GITHUB_RUN_ID-$env:GITHUB_RUN_ATTEMPT"
if (Test-Path -LiteralPath $verifyRoot) { throw 'x64 ZIP 验证目录已存在。' }
...
foreach ($name in $manifestNames) {
    ...
    if ($original -cne $extractedHash) { throw "x64 ZIP 解包哈希不一致：$name" }
}
Assert-X64PE (Join-Path $extracted 'grok-zh.exe')
Assert-X64PE (Join-Path $extracted 'rg.exe')
...
if ($LASTEXITCODE -ne 0 -or $versionOutput -notmatch "^grok-zh $([regex]::Escape($env:GROK_VERSION)) \(") {
    throw "x64 ZIP 解包版本冒烟失败：$versionOutput"
}
$installPreview = Join-Path $env:RUNNER_TEMP "grok-zh-x64-msvc-install-preview-$env:GITHUB_RUN_ID-$env:GITHUB_RUN_ATTEMPT"
